...
def kl_divergence(means, logvars):
    x_mu, y_mu, z_mu = means
    x_logvar, y_logvar, z_logvar = logvars
    x_kl = -0.5*(1.0 + x_logvar - jnp.square(x_mu) - jnp.exp(x_logvar) )
    y_kl = -0.5*(1.0 + y_logvar - jnp.square(y_mu) - jnp.exp(y_logvar) )
    z_kl = -0.5*(1.0 + z_logvar - jnp.square(z_mu) - jnp.exp(z_logvar) )
...
def main_train (json_data):
    ...
    rng, key, z_rng = jax.random.split (rng, num=3)
    
    input_size = x_A.shape[1]*3
    
    latents = 3
    if 'VAE_RNVP' in json_data:
        if json_data['VAE_RNVP']['latents'] > 1:
            latents = json_data['VAE_RNVP']['latents']

    mask_fixed = jnp.array(json_data['realNVP']['mask_fixed']) - 1
    model = flax_nn.VAErealNVP3(input_size=input_size, 
                     latents=latents,
                     fixed_atoms=mask_fixed)
    
    state = train_state.TrainState.create (
            apply_fn=model.apply,
            params=model.init (key, x_A, z_rng)['params'],
            tx=opt_method
        )
    
    @jax.jit
    def train_step1 (state, batchs, z_rng,
        ...
        def loss_fn (params, apply_fn):
            ...
            kld_loss_R = kl_divergence (means_R, logvars_R).mean()

            return loss_wBnd+kld_loss_F+kld_loss_R
    ...
    for epoch in range (json_data['nepoch']):
        ...
        for ist0 in range (0,nconf,1000):
            ...
            state = train_step1 (state, batch, z_rng,
                                ener_batch_ref0, fixed_R0)
